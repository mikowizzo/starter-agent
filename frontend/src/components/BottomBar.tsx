...
import { pruneStoppingRuns } from "../lib/session";
...
export function BottomBar({
  ...
  // Prune stopping markers for runs that are no longer active
  useEffect(() => {
  ...
  const hasActiveRun = liveNow || activeRuns.length > 0;
  ...
  return (
    <div className="relative pb-[calc(0.5rem+env(safe-area-inset-bottom,0px))]">
      ...
      <div className="flex items-center h-10 px-1">
        ...
        {/* Active run indicator — one run per agent now, so a dot suffices.
            Pulsing = a run is executing; hollow = idle. Auto-attaches on load. */}
        <div className="order-first flex items-center px-1.5" title={hasActiveRun ? "A run is active — new tabs attach automatically" : "No active runs"}>
          {hasActiveRun ? (
            <span className="relative flex h-1.5 w-1.5" style={{ ['--glow-color' as string]: 'var(--color-accent)' }}>
              <span className="absolute inline-flex h-1.5 w-1.5 rounded-full bg-[var(--color-accent)] animate-glow-pulse"></span>
              <span className="relative inline-flex h-1.5 w-1.5 rounded-full bg-[var(--color-accent)] animate-dot-glow"></span>
            </span>
          ) : (
            <span className="inline-flex h-1.5 w-1.5"></span>
          )}
        </div>
          ...
          {switcher.clones
            ...
            .map((c) => (
              <InstanceLink
                ...
                runCount={c.name === currentName ? (hasActiveRun ? 1 : 0) : cloneCounts?.[c.name] ?? 0}
              />
            ))}
        </div>
      )}
...
function InstanceLink({
  ...
  // One-run-per-agent: a dot is all the info there is
  const runningDot = runCount ? (
    <span className="relative mr-1 inline-flex h-1.5 w-1.5" style={{ ['--glow-color' as string]: 'currentColor' }}>
      <span className="absolute inline-flex h-1.5 w-1.5 rounded-full bg-current opacity-40"></span>
      <span className="relative inline-flex h-1.5 w-1.5 rounded-full bg-current animate-dot-glow"></span>
    </span>
  ) : null;
  return isCurrent ? (
