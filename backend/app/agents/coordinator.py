...
def build_team(
    ...
    *,
    ...
    instructions = [
        ...
        "TOOL DISAMBIGUATION: 'ask the crew', 'ask crew', or 'ask our models' ALWAYS means run the ask-crew skill (backend/app/skills/ask-crew/ask_crew.py) — it queries the four OpenCode MODELS (MiniMax M3, Kimi K3, Qwen 3.8 Max, GLM 5.2) in parallel via the OpenCode API. It has NOTHING to do with the clone crew members (franky, nami, sanji, zoro, robin, luffy, usopp). Only use the talk_to tool (TeamComms) when the user names a SPECIFIC crew member or wants to chat with a Straw Hat clone. When in doubt, ask-crew = models, talk_to = clones.",
    ]
